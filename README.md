...
5. **代码块语言标注可能对不上** —— 检查一下 ` ```语言名` 是否是 Shiki 认识的语言（`ts`/`tsx`/`glsl`/`bash`/`json` 等常见语言都支持），想显示文件名就加 `title="路径"`。
...
结果按文章分组：标题右边标着这篇里一共命中几处，下面把**每一处命中的句子都列出来**（一篇最多列 6 条，更多的话会注明还有几处）。点任意一条会跳进文章，并且**直接滚到那一句**——页面上所有出现该关键词的地方都会标黄，点中的那处颜色更深。
...
- 文章多起来之后索引会变大（目前 4 篇约 25KB）。如果哪天涨到几百篇导致下载变慢，再考虑换成服务端搜索 API——因为 `next.config.ts` 保留了标准 Next.js 运行时，加 `/api/search` 路由不需要改部署方式。
